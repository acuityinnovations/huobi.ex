defmodule ExHuobi.UsdtSwap.Rest.Public do
  @moduledoc false
  ...
  def get_price_limit(instrument_id) do
    HTTPoison.get(
      "#{@hbdm_host}/linear-swap-api/v1/swap_price_limit?contract_code=#{instrument_id}"
    )
  ...
  def get_best_ticker(instrument_id) do
    HTTPoison.get(
      "#{@hbdm_host}/linear-swap-ex/market/detail/merged?contract_code=#{instrument_id}"
    )
    |> Handler.parse_response()
  end
  ...
  def instruments(instrument_id) do
    HTTPoison.get(
      "#{@hbdm_host}/linear-swap-api/v1/swap_contract_info?contract_code=#{instrument_id}"
    )
    |> Handler.parse_response()
  end
